...
namespace SneakerStore.Controllers
{
    public class ShoppingCartController : Controller
    {
		...
		{
			Cart cart = new Cart();
			Func<double> tinhTongTien = cart.Total_money;
		...
		public List<ViewModel> getCarts()
		{
			var getUser = Session["user"] as Customer;

			List<ViewModel> gioHang = Session["GioHang"] as List<ViewModel>;

			if (getUser != null)
			{
				var getListCart = database.OrderDetails.Where(x => x.OrderPro.IDCus == getUser.IDCus).ToList();
				List<ViewModel> listCart = new List<ViewModel>();
				if (getListCart != null || getListCart.Count > 0)
				{
					foreach (var i in getListCart)
					{
						ViewModel sp = new	ViewModel(i.IDProduct);
						sp.Sum_Quantity = (int)(i.Quantity);
						listCart.Add(sp);
					}	
		...
		{
			double TongTien = 0;
			List<ViewModel> gioHang = getCarts();
			if (gioHang != null)
			{
		...
		public ActionResult PaymentChoice(string paymentMethod, string DiaDiemGiaoHang)
		{
			...
			{
				_context.SetChoice(new PayPalChoice(() => TinhTongTien()));
			}
